...
if [ ! -v OWNER ]; then
    echo "## ERROR: incorrect usage of CI. OWNER (e.g. dockerhub organization like pcrespov or user private name) not defined!"
    exit 1
fi

# only upstream is allowed to push to pcrespov repo
if [ "${OWNER,,}" != "pcrespov" ] &&\
    { [ ! -v DOCKER_REGISTRY ] || [ -z "${DOCKER_REGISTRY}" ] || [ "$DOCKER_REGISTRY" = "pcrespov" ]; }; then
